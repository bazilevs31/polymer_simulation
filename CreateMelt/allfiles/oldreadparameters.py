...
def read_traj_vmd():
    ...
    if (os.path.isfile(args.psffile+'.psf')) and (os.path.isfile('trajSkipwrap.dcd')):
        args.traj = 'trajSkipwrap.dcd'
        args.trajskip = 1
    else:
...
def create_psf(args):
    ...
    args.psffile = os.path.splitext(args.psffile)[0]
    curdir = os.getcwd()
    curdir = curdir+"/"
    filestring = " ".join(("create_list_cryst.sh",args.datafile,args.traj,str(args.trajskip),args.psffile,curdir))
